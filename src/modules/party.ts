...
function renderTally(): void {
  ...
  document.getElementById("tally-members")!.textContent =
...
function buildCard(pc: PCCard): HTMLElement {
  ...
  nameInput.style.cssText =
    'font-family:"Cinzel",serif;font-size:14px;font-weight:600;color:var(--gold-pale);letter-spacing:0.04em;text-align:left;';
  nameInput.addEventListener("change", () => {
    draft.name = nameInput.value.trim() || "Unnamed";
    flush();
  });
  ...
  const saveKeys: Array<keyof PCCard["saves"]> = [
    "str",
    "dex",
    "con",
    "int",
  ...
  saveKeys.forEach((key) => {
    ...
    val.addEventListener("change", () => {
      draft.saves[key] = val.value.trim();
      flush();
    });
  ...
  body.appendChild(
    makeFieldRow("Perception", draft.passives.perception, (v) => {
      draft.passives.perception = v;
      flush();
    }),
  );
  body.appendChild(
    makeFieldRow("Insight", draft.passives.insight, (v) => {
      draft.passives.insight = v;
      flush();
    }),
  );
  body.appendChild(
    makeFieldRow("Investigation", draft.passives.investigation, (v) => {
      draft.passives.investigation = v;
      flush();
    }),
  );
  ...
  body.appendChild(
    makeCurrencyRow("Platinum", draft.currency.platinum, (v) => {
      draft.currency.platinum = v;
      flush();
    }),
  );
  body.appendChild(
    makeCurrencyRow("Gold", draft.currency.gold, (v) => {
      draft.currency.gold = v;
      flush();
    }),
  );
  ...
  addFieldBtn.addEventListener("click", () => {
    ...
    setTimeout(() => {
      const inputs =
        customList.querySelectorAll<HTMLInputElement>(".pc-custom-name");
...
export function initParty(): void {
  document
    .getElementById("btn-add-pc")!
    .addEventListener("click", openAddPCModal);

  document
    .getElementById("btn-confirm-add-pc")!
    .addEventListener("click", confirmAddPC);

  document
    .getElementById("btn-cancel-add-pc")!
  ...
  document
    .getElementById("new-pc-name-input")!
