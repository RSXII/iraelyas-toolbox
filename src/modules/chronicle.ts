...
export function exportTimelineFile(): void {
  ...
  if (!data) { showToast('No timeline data to export'); return; }
...
export function renderChronicle(): void {
  ...
  if (!data) {
    root.innerHTML = '<div class="tl-empty">No timeline data — import a timeline-data.js file above.</div>';
  ...
  const packedSections = data.sections.map(s => ({ section: s, rows: packRows(s.items) }));
  ...
  requestAnimationFrame(() => {
    ...
    scrollToNow();
    document.getElementById('tl-jump-now')?.addEventListener('click', scrollToNow);
  });
}

// ── Item factory ───────────────────────────────────────────────────────────

function makeItem(item: TimelineItem, tint: TintConfig, startYear: number): HTMLElement {
  if (item.type === 'point') {
...
function alignLabels(labelCol: HTMLElement, gridInner: HTMLElement): void {
  ...
  gridInner.querySelectorAll<HTMLElement>('.tl-section-grid').forEach(grid => {
    const id  = grid.dataset.sectionId!;
    const lbl = labelCol.querySelector<HTMLElement>(`[data-section-id="${id}"]`);
    if (!lbl) return;
