...
export function importHouseFile(): void {
  window.toolbox.importFile([{ name: 'JSON', extensions: ['json'] }])
    .then(files => {
      ...
      try {
        ...
        const key = (data.house || 'house').toLowerCase().replace(/[^a-z0-9]+/g, '_');
...
export function initTree(): void {
  ...
  if (!cid || !houseId) {
    titleEl.textContent = '—';
    container.innerHTML = '<div class="empty-state" style="padding:3rem">Import a house JSON to view the family tree.</div>';
    return;
  }
...
function buildSVG(data: HouseData): void {
  ...
  const COL0      = spine?.enabled ? (layout?.col_start ?? 290) : (layout?.col_start ?? 120);
  const PAD_TOP   = 34;
  const PAD_SIDE  = 20;
  ...
  function rowY(row: number) { return PAD_TOP + (row - 1) * ROW_H + ROW_H / 2; }
  ...
  members.forEach(m => { pos[m.id] = { x: colX(m.col), y: rowY(m.row) }; });
  ...
  const bioCols = members.filter(m => m.col !== 'spine').map(m => m.col as number);
  ...
  function el(tag: string, attrs: Record<string, string | number> = {}, text = ''): SVGElement {
  ...
  for (let r = 1; r <= maxRow; r++) {
    const y = rowY(r);
    if (r % 2 === 0) {
      gBg.appendChild(el('rect', { x: 0, y: y - NH/2 - 10, width: svgW, height: NH + 20, fill: 'rgba(255,255,255,0.012)' }));
    }
    gBg.appendChild(el('text', { x: spine?.enabled ? 8 : 6, y: y + 4, fill: '#252528', 'font-family': 'Cinzel,serif', 'font-size': 9, 'letter-spacing': '0.06em' }, toRoman(r)));
  }

  // Spine divider
  if (spine?.enabled) {
    gBg.appendChild(el('line', { x1: SPINE_X + NW/2 + 18, y1: PAD_TOP - 14, x2: SPINE_X + NW/2 + 18, y2: svgH - 20, stroke: '#1a2a3a', 'stroke-width': 1, 'stroke-dasharray': '3 7' }));
    const sl = el('text', { x: SPINE_X, y: PAD_TOP - 14, 'text-anchor': 'middle', 'font-family': 'Cinzel,serif', 'font-size': 8, 'letter-spacing': '0.1em', fill: spineColor, opacity: 0.7 });
  ...
  function nodeStyle(type: string) {
    switch (type) {
      case 'priestess': return { fill: '#0d2b43', stroke: spineColor, sw: 1.5 };
      case 'current':   return { fill: '#0d2b43', stroke: '#4caf50',  sw: 2.5 };
      case 'heir':      return { fill: '#091e30', stroke: spineColor, sw: 1.5 };
      case 'unknown':   return { fill: '#111114', stroke: '#383530',  sw: 1.5 };
      default:          return { fill: '#14141a', stroke: '#35322c',  sw: 1.5 };
    }
  }
  ...
  members.forEach(n => {
    ...
    gLines.appendChild(el('path', { d: `M${px},${startY} L${px},${midY} L${child.x},${midY} L${child.x},${endY}`, fill: 'none', stroke: '#3a3530', 'stroke-width': 1.5, 'marker-end': 'url(#arr-bio)' }));
  });
  ...
  if (spine?.enabled) {
    ...
    for (let i = 0; i < spineNodes.length - 1; i++) {
      ...
      gLines.appendChild(el('line', { x1: SPINE_X, y1: pa.y + NH/2, x2: SPINE_X, y2: pb.y - NH/2, stroke: spineColor, 'stroke-width': 1.5, 'stroke-dasharray': '6 4', 'marker-end': 'url(#arr-adoptive)' }));
    }
  }
  ...
  if (spine?.enabled) {
    members.forEach(n => {
      ...
      const ls: Record<string, string | number> = { stroke: '#1a5a8a', 'stroke-width': 1.5, 'stroke-dasharray': '5 4' };
      ...
      gLines.appendChild(el('line', { x1: SPINE_X, y1: dropY, x2: SPINE_X, y2: spinePos.y - NH/2, ...ls, 'marker-end': 'url(#arr-origin)' }));
    });
  }
  ...
  members.forEach(n => {
    ...
    if (n.img) {
      ...
      sg.appendChild(el('path',   { d: `M${scx-PS*0.2},${py2+PS*0.96} Q${scx-PS*0.12},${py2+PS*0.58} ${scx},${py2+PS*0.56} Q${scx+PS*0.12},${py2+PS*0.58} ${scx+PS*0.2},${py2+PS*0.96}`, fill: st.stroke }));
      g.appendChild(sg);
    }
